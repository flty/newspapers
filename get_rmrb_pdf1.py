...
global BASEDIRBASEDIR 

def config():
	global BASEDIR
	cf = ConfigParser.RawConfigParser()
	if not os.path.exists("rmrb.conf"):
		print "System initting, please enter your dir where you want to restore the pdf files:",
		s = raw_input(">")
		print s
		cf.add_section("RESTORE")
		cf.set("RESTORE", "DIR", s)
		cf.write(open("rmrb.conf", "w"))
		BASEDIR = s
		if not os.path.isdir(BASEDIR):
			os.mkdir(BASEDIR)		
	else:
		cf.read("rmrb.conf")
		s = cf.get("RESTORE", "DIR")
		print s
		BASEDIR = cf.get("RESTORE", "DIR")
		print BASEDIR
	return 0
